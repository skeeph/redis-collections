...
public class ToArray {
  ...
  private static void testMap(Map<Integer, Long> map) {
    ...
    for(int each = 0; each < TEST_SIZE; each++) {
      check( "unexpected key", keys[each].equals(KEYS[each]));
    }
    ...
    for(int each = 0; each < TEST_SIZE; each++) {
      check( "unexpected value", values[each].equals(VALUES[each]));
    }
    ...
    for(int each = 0; each < TEST_SIZE; each++) {
      try{
      check( "unexpected entry", entries[each].getKey().equals(KEYS[each]) && entries[each].getValue().equals(VALUES[each]));
